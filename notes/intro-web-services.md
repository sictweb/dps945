...
In class, we will discuss some HTTP topics, recall your BTI325 experience, refer to the [overview on Wikipedia](https://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol), and the official specification series, [RFC 7230](https://tools.ietf.org/html/rfc7230) to RFC 7235, which replaced (or updated) RFC 2616 in June 2014.

<br>

### How do I learn to create web services?

In the previous web programming courses, you have had some coverage and experience. 
...
Returning to the knowledge and skills that you gained in the previous web programming courses, you can map to the list above:
* The requestor was a JavaScript *XmlHttpRequest object* or *Fetch API request* that was running in a browser
* The responder was a *Node+Express web app* that was running in a server
* You learned some fundamentals of the HTTP protocol, including using the GET and POST methods, as well as using other methods (PUT, DELETE, OPTIONS)
* You also learned about responses from the server, and what to do when an HTTP 200 or HTTP 404 response comes back 
* JSON was the data interchange format used in the requests and responses

<br>

**Web services development and deployment environment**

You will continue to use the frameworks and tools from the previous web programming courses:
* Node.js
* Express.js
* MongoDB
* Visual Studio Code
...
A *representation* is a *digital asset* that is *formatted as a specific data interchange format*.
...
Every representation is defined by an data interchange format.
...
[This Wikipedia article](https://en.wikipedia.org/wiki/Media_type) is an acceptable introduction to data interchange formats.
...
* In JSON, there is no Date type. Dates are expressed as strings, almost always in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format. In contrast, JavaScript does have a Date object (not a *type*, but an *object*).
...
> The web services that you create in this course will rely on the JSON data interchange format.

Recognizing JSON is a skill that you ought to develop. It is important and useful to be able to quickly identify the kind of data that's represented in JSON, and the structure of the data. 

To get started, JSON returned from a web service will almost always begin with a brace `{` or a bracket `[`. 
* If it begins with a brace, you're looking at an *object* 
* If it begins with a bracket, you're looking at a *collection* of objects

Outside of an app like Postman, JSON returned from a web service will not be formatted with whitespace and line feeds. However, sometimes it is useful to visualize it in a nicely-formatted manner. How? You have at least two choices:
1. Paste the JSON into a JavaScript document in Visual Studio Code, select all, and then format it (with Shift + Option + F, or Shift + Alt + F on Linux and Windows). 
2. Paste the JSON into an online formatter, for example [JSON Formatter](https://jsonformatter.curiousconcept.com/). 

In addition, for *GET* requests only, you can use a browser. Most browsers support an add-in (or similar implementation) that will nicely format JSON. 

<br>

**State, data**

Let's discuss *state* in this section of the document, and in the next section. 
...
Therefore, if the interaction state of a (message exchange) session is important to maintain, *it's done by the client app*.

<br>

**REST**

Now we talk a little about "REST", which is an acronym for **RE**presentational **S**tate **T**ransfer.

Someone with limited web service development experience may equate REST to a web service that works with JSON. It's much more than that. 

If a student is serious about a career that includes  programming with web technologies, then it is essential to study Roy Fielding's PhD dissertation from the year 2000, [Architectural Styles and the Design of Network-based Software Architectures](http://www.ics.uci.edu/~fielding/pubs/dissertation/top.htm).

> More information about [Roy Fielding is here](https://en.wikipedia.org/wiki/Roy_Fielding).

Will we be creating "REST APIs"? Yes, if we follow all the guidance and practices. However, that will come only with more knowledge and experience, some of which is gained in this academic course. However, we probably won't be able to get all the way there, due to time constraints and other course objectives. 
...
The term "web API" captures the essence of "REST API" above, but it uses a more generic and easier to understand or explain word. 

Arguably, every computer user has a reasonably accurate knowledge of the word "web", so that word doesn't have to be explained. The expansion of the "API" initialism is also easy to understand or explain. It's just a bit better (and safer) than using "REST", and avoids the immediate need to cover large chunks of a [180-page PhD dissertation](http://www.ics.uci.edu/~fielding/pubs/dissertation/fielding_dissertation.pdf). 

So, let's define "web API" as *an API to a web service*.
...
We were reminded of our introduction to and work withweb service programming in the previous academic course.

Terminology was introduced, argued, and explained. 

Now, you're ready to take the next step, and built uppon your previous experience. 
